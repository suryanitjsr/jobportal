...
function SearchbyComp() {
  ...
  async function searchCompany(key){
    let result = await fetch(
        `http://localhost/jobportal/jobportal_backend/public/api/searchbycomp/${key}`
      );
  ...
  return (
    <div>
      ...
      <div className="col-sm-8 offset-sm-2">
        ...
        <br/>
        <input type="text" onChange = {(e) => searchCompany(e.target.value)} placeholder = "Search Company" className = "form-control"/>
        <br/>
        <br/>
        {data.map((item) => (
          <>
